    ...
    sections: [
      {
        ...
        items: [
          ...
          '文件名与标签：检测扩展标签重复与扩展字段、文件名繁体，支持批量清理与繁转简',
          ...
          '关于：新增「音频知识」Tab，介绍 ID3 标签、Vorbis Comment、常见格式与码率等',
          '播放：播放前检测编码；ALAC、WMA、APE 等不支持格式给出明确提示',
    ...
    sections: [
      {
        title: '新增',
        items: [
          '歌词查看：LRC 归位、音乐解密、乐库管理等列表点击「有」标签可弹窗预览同级歌词；支持 LRC 文件头与网易云 JSON 元数据（作词/作曲/编曲及艺人头像）',
          '歌词查看：时间轴与正文分色展示，元数据块与歌词正文以分隔线区分',
          '乐库管理：目录树工具栏可切换仅当前目录 / 含全部子文件夹列表范围',
          '重复清理：识别同名编号副本（如 A.flac 与 A(1).flac）；封面对比弹窗横向并排、固定封面尺寸',
          '文件名与标签：按艺人分隔规范检测；按问题类型筛选，每项可单独执行；有选中时仅处理选中且符合条件的记录',
          '文件名与标签：检测扩展名前尾下划线及文件名 / 标签艺人、曲名首尾下划线，合并为「文件名下划线」「标签下划线」两类并支持批量修复',
          '列表：各音频列表右击可选「打开文件所在位置」',
          '批处理：长时间任务支持取消；左下角悬浮进度面板统一展示进度与取消按钮'
        ]
      },
      {
        title: '变更',
        items: [
          '导航：顶栏「标签校准」更名为「文件名与标签」',
          '文件名与标签：文件名艺人重命名若目标已存在自动追加 (1)、(2)… 后缀；批量进度不再显示上个约 / 本首约用时',
          '元数据面板：多选时不显示，仅单选一条时展示',
          '重复清理 / 文件名与标签：扫描来源下拉框占满侧栏宽度',
          '乐库同步：对比与复制过程显示进度，单文件复制也显示进度',
          '界面：Tooltip / Popover 使用 12px 小字',
          '重复清理：组内重复项纵向列表；切换保留项时封面对比布局不再跳动'
        ]
      },
      {
        title: '修复',
