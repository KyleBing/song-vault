...
| 4 | 曲库同步 | 对比左右两个曲库目录的音频差异（相同、仅一侧、大小不同、已移动），支持跨库复制与同库内移动对齐路径 |
| 5 | 设置 | LRC 源、音频目标、曲库目录、路径过滤规则等配置 |
...
1. 在 **设置 → 同步设置** 中指定左右曲库根目录（可设置显示别名）
2. 打开 **曲库同步** 工作台，点击 **扫描对比**
3. 在差异树中查看统计与明细；**已移动** 表示两侧均有同名同大小音频但相对路径不同
4. 操作方式：
   1. 仅一侧有文件：箭头指向的一侧从另一侧**复制**缺失文件
   2. 已移动：在箭头指向的曲库内**移动**（rename）文件以对齐另一侧路径，同名 `.lrc` 会一并移动
5. 支持 Shift 连选与批量同步
